...
export class CardManager {
    ...
    EventBus.on('roll:complete', async (playerOrder) => {
      delayAndAWait(async () => {
        for (let i = 0; i < this.cards.length; i++) {
          const playerData = playerOrder.find((p) => p.uuid === this.cards[i].uuid)
          if (playerData) {
  ...
  setPolygonShape() {
    ...
    for (let i = 0; i < this.cards.length; i++) {
      this.cards[i].desiredPosition = new THREE.Vector3(coordinates[i].x, coordinates[i].y, 0)
      this.cards[i].desiredRotation = new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, 0))
    }
  }
