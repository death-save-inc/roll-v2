...
export class DieManager {
  constructor(controller, players = []) {
    ...
    window.addEventListener('resize', () => {
      ...
      if (this.outlineLine?.material) this.outlineLine.material.resolution.set(w, h)
    })
  ...
  setPolygonShape() {
    ...
    for (let i = 0; i < this.dice.length; i++) {
      this.dice[i].setPosition(new THREE.Vector3(this.coordinates[i].x, this.coordinates[i].y, 3))
      this.dice[i].desiredRotation = new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0, 0))
    }
    // this.connectDice();
  }
  ...
  async roll() {
    ...
    allPlayers.forEach(async (player) => {
      if (player) {
        // Ensure player is not undefined or null
        player.reroll = null
