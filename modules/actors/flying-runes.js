...
export class FlyingRunes {
  ...
  _buildVFX() {
    ...
    // GLYPHS
    const glyphs = ['ᚠ', 'ᚢ', 'ᚦ', 'ᚨ', 'ᚱ', 'ᚲ', 'ᚷ', 'ᚹ', 'ᚺ', 'ᚾ', 'ᛁ', 'ᛃ', 'ᛈ', 'ᛇ', 'ᛉ', 'ᛊ']
    ...
    glyphs.forEach((ch, i) => {
      ...
      const rad = 5
      sprite.position.set(Math.cos(ang) * rad, this.basePositionY, Math.sin(ang) * rad)
  ...
  _collectFadeables() {
    ...
    const hasFadeUniform = (m) => m && m.uniforms && m.uniforms.fadeFactor !== undefined
  ...
  _update() {
    ...
    this.fadeables.forEach(({ mat, useOpacity }) => {
      if (mat.uniforms?.fadeFactor !== undefined) mat.uniforms.fadeFactor.value = f
