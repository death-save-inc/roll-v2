...
export class Die {
    ...
    if (this.dieParent) {
      this.dieParent.traverse((child) => {
        if (child.material && child.material.opacity !== undefined) {
          const distanceToTargetOpacity = Math.abs(child.material.opacity - this.targetOpacity)
  ...
  async createDieMesh() {
    const texture = await this.controller.loadTexture('assets/models/textures/d20-inverted.png')
  ...
  update() {
    if (this.dieParent) {
      ...
      this.dieParent.quaternion.copy(this.controller.cameraController.camera.quaternion)
