...
export class Card extends Actor {
  ...
  async init() {
    ...
    this.controller.interactions.push(
      new Interaction(this.mesh.uuid, this.onClick.bind(this), this.onHover.bind(this)),
    )
  ...
  async generateCardTexture(idx) {
    ...
    const centerY = innerY + innerHeight / 2 + (metrics.actualBoundingBoxAscent - textHeight / 2)
    ...
    ctx.save()
    ctx.beginPath()
    ctx.ellipse(canvas.width / 2, canvas.height / 2, xRadius + 20, yRadius, 0, 0, Math.PI * 2)
    ctx.closePath()
    ctx.clip()
    ...
    ctx.fillText(idx | 0, padding, size.actualBoundingBoxAscent + padding * 1.5)
  ...
  async generateCard() {
    ...
    console.log(`Card mesh created for ${this.name} (${this.uuid}) at position:`, this.position)
  }
