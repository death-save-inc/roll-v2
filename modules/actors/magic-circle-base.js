...
export class MagicCircleBase {
  constructor(controller) {
    ...
    controller.registerRenderAction('MagicCircleFade', /*order=*/ 1, () => this.update())
  }
  ...
  makeLineMats() {
    // outer circle
    ;[this.circleMat, this.circleBloomMat] = this._makeLineMats(COLORS.circle, 4)
  ...
  addPentagramWithCircles(parent, R, rSmall, segs) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      tips.push(new THREE.Vector3(R * Math.cos(a), this.baseYPosition, R * Math.sin(a)))
    }
    ...
    tips.forEach((v) => {
      ...
      for (let j = 0; j <= ringSegs; j++) {
        ...
        ring.push(v.x + rSmall * Math.cos(t), this.baseYPosition, v.z + rSmall * Math.sin(t))
      }
  ...
  _createFireEffect() {
    this.fire = new FireEffect(this.controller, new THREE.Vector3(0, 4, 0), 1, 10, 400, 0.5, 0.0125)
  }
}
