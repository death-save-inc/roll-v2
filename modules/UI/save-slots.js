...
export class SaveSlotsUI {
  ...
  async renderSaveSlotsFromLocalStorage() {
    ...
    for (const slotEntry of slotsArray) {
      ...
      const slotName =
        (slotEntry && (slotEntry.slotName || slotEntry.name)) ||
        (typeof slotEntry === 'string' ? slotEntry : String(slotEntry))
      ...
      if (!slotData) {
        ...
        const storageKey =
          slotEntry && slotEntry.storageKey
            ? slotEntry.storageKey
          ...
          const displayName =
            typeof player.name === 'string'
              ...
              : player.name &&
                  ...
                  typeof player.name.name === 'string'
                ? player.name.name
                : JSON.stringify(player.name)
          const imgSrc =
            player.imageSrc && typeof player.imageSrc === 'string'
              ? player.imageSrc
              : 'default-image.png'
