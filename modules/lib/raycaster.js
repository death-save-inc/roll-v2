...
export class Raycaster {
  ...
  init() {
    ...
    window.addEventListener('raycaster:active', () => {
      this.active = true
      console.log('active!')
    })
    window.addEventListener('raycaster:inactive', () => {
      this.active = false
      console.log('inctive!')
    })
  }
      ...
      for (let i = 0; i < this.intersects.length; i++) {
        if (this.intersects[i].object.type === 'Mesh') {
          if (this.intersects[i].object) {
            const action = this.controller.interactions.find(
              (action) => action.id === this.intersects[i].object.uuid,
            )
