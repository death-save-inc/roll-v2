...
export class LocalStorageManager {
  ...
  createSlot(slotName) {
    ...
    try {
      ...
      const currentPlayers =
        currentSave &&
        currentSave.data &&
        Array.isArray(currentSave.data.players) &&
        currentSave.data.players.length
          ? JSON.parse(JSON.stringify(currentSave.data.players))
          : null
      if (currentPlayers) {
        ...
        for (let i = 1; i <= 5; i++) {
          const uuid =
            typeof crypto !== 'undefined' && crypto.randomUUID
              ...
              : `player-${Date.now()}-${Math.floor(Math.random() * 100000)}`
          defaultPlayers.push({
            name: `Player ${i}`,
            uuid: uuid,
            imageSrc: null,
      ...
      console.warn(
        'Failed to initialize players for new slot, defaulting to empty players array',
        err,
      )
      slot.data.players = []
    }
    ...
    try {
      console.debug(
        `createSlot: persisting new slot '${slotName}' -> key '${slot.storageKey}'`,
        slot.data,
      )
      slot.save(slot.data)
    } catch (err) {
  ...
  saveToExistingSlot(path, value) {
    ...
    console.debug(
      `saveToExistingSlot: path='${path}' value=`,
      value,
      `on slot='${this.saveSlotName}' (storageKey='${slot.storageKey}')`,
    )
    slot.set(path, value)
    try {
      ...
      try {
        ...
        if (raw) {
          ...
          try {
            ...
            console.warn(
              `getAllSlotsFromStorage: non-JSON payload for key ${key}, storing raw value under .data`,
            )
            slot.data = { raw: raw }
          }
        }
